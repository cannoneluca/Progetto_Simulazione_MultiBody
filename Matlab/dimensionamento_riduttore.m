...
riduttore.fs = riduttore.fs(ceil(mean(riduttore.V2*60)));
...
for k5 = 1:length(catalogo_riduttori(:,1))
    ...
    % Verifica che non si superi la velocità massima di ingresso
    catalogo_riduttori(k5,10) = max(abs(riduttore.V2))/riduttore.tau*simulation.safety <= riduttore.N1max; % Limite massimo
    % si verifica che la velocità media non superi quella nominale
    ...
    % verifica che non sia superato il limite relativo alla coppia massima
    catalogo_riduttori(k5,12) = catalogo_riduttori(k5,2) > riduttore.T2B;
    % verifico che la coppia rms non superi la coppia nominale
    % tale limite va considerato solo se il duty cycle è superiore al 60 %
    catalogo_riduttori(k5,13) = catalogo_riduttori(k5,3) > riduttore.T2M + (riduttore.DC < 60);
