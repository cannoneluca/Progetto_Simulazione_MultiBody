...
motore.V = CB.fp*30/pi/riduttore.tau; % [rpm] Velocità angolare del motore
...
for k6 = 1:length(catalogo_motori)
    if(motore.OK == false)
        ...
        motore.Max_Torque_Speed = catalogo_motori(k6,6); % [rpm]
        motore.Rated_Speed = catalogo_motori(k6,7); % [rpm]
        motore.Max_Speed = catalogo_motori(k6,8); % [rpm]
